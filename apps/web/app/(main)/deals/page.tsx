...
export default function DealsPage() {
  ...
  const deals = products?.filter((p) => p.comparePrice && p.comparePrice > p.price) ?? [];
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--shop-bg)" }}>
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20 py-16">
        <p className="text-[10px] tracking-[0.22em] uppercase text-[color:var(--shop-muted)] mb-2">Limited time</p>
        ...
        {!isLoading && deals.length === 0 && (
          <div className="py-24 text-center">
            <p className="font-display text-3xl text-[color:var(--shop-muted)] italic mb-4">No deals right now</p>
              ...
              return (
                <Link key={p.id} href={`/products/${p.id}`} className="group block shop-card">
                  <div className="relative overflow-hidden rounded-xl bg-[color:var(--shop-overlay)] aspect-[3/4]">
                    ...
                    <span className="absolute top-3 left-3 text-xs font-bold bg-[color:var(--color-primary-600)] text-white px-3 py-1 rounded-full">
                      −{discount}% OFF
                    </span>
                  </div>
                  <div className="mt-3">
                    ...
                    <div className="flex items-baseline gap-2 mt-1">
                      <span className="font-mono-price text-sm font-bold text-gray-900 dark:text-gray-100">{fmt(p.price)}</span>
                      <span className="font-mono-price text-xs text-[color:var(--shop-muted)] line-through">{fmt(p.comparePrice!)}</span>
                    </div>
                  </div>
                </Link>
              );
